...
#define BL_REQ_PIN            PIN_PA19

#define UART_TX_PORT          0 // PA
#define UART_TX_PIN           PIN_PA16C_SERCOM1_PAD0
#define UART_TX_MUX           MUX_PA16C_SERCOM1_PAD0

#define UART_RX_PORT          0 // PA
#define UART_RX_PIN           PIN_PA17C_SERCOM1_PAD1
#define UART_RX_MUX           MUX_PA17C_SERCOM1_PAD1

#define UART_SERCOM           SERCOM1
#define UART_SERCOM_GCLK_ID   SERCOM1_GCLK_ID_CORE
#define UART_SERCOM_MASK_REG  APBCMASK
#define UART_SERCOM_MASK_BIT  MCLK_APBCMASK_SERCOM1
...
static bool bl_request(void)
{
  ...
  delay_cycles(1 * (4000000 / 1000)); // 1 ms, CPU frequency is 4 MHz at this point
...
static void uart_init(void)
{
  ...
  GCLK->PCHCTRL[UART_SERCOM_GCLK_ID].reg = GCLK_PCHCTRL_GEN(0) |
      GCLK_PCHCTRL_CHEN;
...
static void command_task(void)
{
  ...
  else if (BL_CMD_UNLOCK == uart_command)
  {
    ...
    bool pass = (end > begin) && ((end <= FLASH_SIZE) ||
        (begin >= DATA_FLASH_ADDR && end <= (DATA_FLASH_ADDR + DATA_FLASH_SIZE)) ||
